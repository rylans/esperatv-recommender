module.exports = [
  {name: 'inception', keywords: ['action', 'adventure', 'sci-fi', 'christopher nolan', 'leonardo dicaprio']},
  {name: 'the wolf of wall street', keywords: ['biography', 'crime', 'drama', 'martin scorsese', 'leonardo dicaprio', 'jonah hill']},
  {name: 'mad max: fury road', keywords: ['action', 'adventure', 'sci-fi', 'george miller', 'tom hardy', 'charlize theron']},
  {name: 'toy story 3', keywords: ['animation', 'adventure', 'comedy', 'lee unkrich', 'tom hanks', 'tim allen', 'joan cusack']},
  {name: 'dunkirk', keywords: ['action', 'drama', 'history', 'christopher nolan', 'fionn whitehead', 'barry keoghan']},
  {name: 'the hateful eight', keywords: ['crime', 'drama', 'mystery', 'quentin tarantino', 'samuel l jackson', 'kurt russel']},
  {name: 'the martian', keywords: ['adventure', 'drama', 'sci-fi', 'ridley scott', 'matt damon', 'jessica chastain']},
  {name: 'django unchained', keywords: ['drama', 'western', 'quentin tarantino', 'jamie foxx', 'christoph waltz', 'leonardo dicaprio']},
];
